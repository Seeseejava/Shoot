...
#include "Shoot/Character/ShootAnimInstance.h"
#include "Shoot/Weapon/Projectile.h"
#include "Shoot/Weapon/Shotgun.h"
...
void UCombatComponent::OnRep_EquippedWeapon()
{
	...
	{
		// In order to ensure Setweaponstate before AttachActor on Client, since both of them are replicated.
...
void UCombatComponent::TraceUnderCrosshairs(FHitResult& TraceHitResult)
{
	...
	if (bScreenToWorld)
	{
		...
		// in order to not shoot the character itself or chracter behind it.
		if (Character)
		{
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		HUD = HUD == nullptr ? Cast<AShootHUD>(Controller->GetHUD()) : HUD;
		if (HUD)
		{
			if(EquippedWeapon)
			{
...
void UCombatComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (Character && Character->IsLocallyControlled())
	{
		...
		if (HitResult.ImpactPoint.Size() > 0.f)
		{
...
void UCombatComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(UCombatComponent, CarriedAmmo, COND_OwnerOnly); // save bandwidth
...
void UCombatComponent::ServerLaunchGrenade_Implementation(const FVector_NetQuantize& Target)
{
	if (Character && GrenadeClass && Character->GetAttachedGrenade())
	{
		...
		SpawnParams.Instigator = Character; // Important!
